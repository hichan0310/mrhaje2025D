...
namespace PlayerSystem
{
    ...
    public class PlayerActionController : MonoBehaviour
    {
        ...
        [SerializeField]
        [Tooltip("Layers considered solid ground for jumping and landing checks.")]
        private LayerMask groundLayers = 0;

        [Header("Drop Through Platforms")]
        [SerializeField]
        [Tooltip("Layers that can be toggled off temporarily when the player drops through platforms.")]
        ...
        private void EnsureDefaultLayerMasks()
        {
            if (groundLayers == 0)
            {
                groundLayers = LayerMask.GetMask("Ground", "Default", "OneWayPlatform");
                if (groundLayers == 0)
                {
            ...
            if (dropThroughLayers == 0)
            {
                dropThroughLayers = LayerMask.GetMask("OneWayPlatform");
            }
        }
        ...
        {
            BindAction(moveAction, OnMovePerformed, OnMovePerformed, OnMoveCanceled);
        ...
        private static Vector2 ReadVector2(InputAction action)
        {
            ...
            if (type == typeof(Vector4))
            {
                var value = action.ReadValue<Vector4>();
                return new Vector2(value.x, value.y);
            }
            ...
            if (type == typeof(int))
            {
                return new Vector2(action.ReadValue<int>(), 0f);
            }
